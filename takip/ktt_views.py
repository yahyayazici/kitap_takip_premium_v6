...
def _ktt_rapor_analiz_verisi(request):
    ...
    return analiz, sonuclar_list


@login_required
@require_permission("ktt", "view")
def ktt_rapor_analiz(request):
    analiz, _ = _ktt_rapor_analiz_verisi(request)
    return render(
        request,
        "includes/ktt_analiz_web.html",
        {
            "analiz": analiz,
            "analiz_durumu": ktt_analiz_durumu(),
        },
    )
...
def ktt_rapor(request):
    ...
    analiz_params = request.GET.copy()
    analiz_url = f"{reverse('ktt_rapor_analiz')}?{analiz_params.urlencode()}"
